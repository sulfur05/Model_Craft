...
def dataset_eda(df: pd.DataFrame, numeric_cols, categorical_cols):
    ...
    if categorical_cols:
        ...
        if df[cat_col].size > 0:
            ...
            # 🔴 CHANGE 7: Class imbalance detection
            target_column = st.session_state.get("target_column")
            if target_column == cat_col and share > 0.7:
                st.warning(
                    f"⚠️ **Class Imbalance Detected**: Your target column is imbalanced! "
                    f"{share:.0%} of samples belong to '{top}' class."
                )
                st.info(
                    "💡 **What does this mean?** When classes are imbalanced, the model may favor the majority class. "
                    "During training, we'll automatically adjust class weights to handle this."
                )
